...
/*
	 * Initializes the state of the processor by setting all initial instructions to NOOP, and * all registers to zero.
 */
...
/*
	 * Read machine code file into instrMem.
*/
...
/*
	...
	* INPUT:
	* OUTPUT: IFID.instr; IFID.pcPlus1;
*/
...
/*
	...
	* INPUT: IFID.instr; IFID.pcPlus1;
	* OUTPUT: IDEX.instr; IDEX.pcPlus1; IDEX.readRegA; IDEX.readRegB; IDEX.offset;
 */
...
/*
	* Execute Stage.
	* INPUT: IDEX.instr; IDEX.pcPlus1; IDEX.readRegA; IDEX.readRegB; IDEX.offset;
	* OUTPUT: EXMEM.instr; EXMEM.branchTarget; EXMEM.aluResult; EXMEM.readRegB;
*/
void execute( const stateType state, stateType* newStatePtr )
{
	...
	// Pass the instruction through first, in order to allow cmP to fill with a noop.
	output.instr = input.instr;
	...
	switch( opcode(state.IDEX.instr) )
	{
		...
		case CMP:
		/*
			* CMP is treated as faithfully as an R-Type instr. as it possibly can (which 
            * becomes the writeData of R-Type instr.) 
            * Will be regA if the condition is satisfied. In order to prevent any sort
			* of special cases and/or modifications to the given stateType
			* struct, a NOOP is sent to EXMEM. 
		*/ 
			if( dataRegB != 0 ) {
...
/*
	...
	* INPUT: EXMEM.instr; EXMEM.branchTarget; EXMEM.aluResult; EXMEM.readRegB;
	* OUTPUT: MEMWB.instr; MEMWB.writeData;
*/
...
/*
	* Writeback Stage.
	* INPUT: MEMWB.instr; MEMWB.writeData;
	* OUTPUT:WBEND.instr; WBEND.writeData;
 */
...
int detectAndStall( const stateType state, IDEXType* IDEX ) 
{
	...
	return( opcode(state.IDEX.instr) == MOVL 
			&& (regA == field1(state.IDEX.instr) || regB == field1(state.IDEX.instr)) );
}

/*
	*	Pipeline registers is checked for data hazards
	*	to increasing recency so that we can forward
...
void forward( const stateType state, IDEXType* IDEX )
{
	...
	switch( opcode(state.WBEND.instr) )
	{
		...
		*/
		...
		/*
			...
			* field1 has already been loaded, and forward the value to the pipeline register
		*/
	...
	switch( opcode(state.EXMEM.instr) )
	{
		...
		*/
		...
		case MOVL:
		/*
			* If a MOVL is dected in the EXMEM pipeline, then the CPU has only
...
/*
	* Since branches are always predicted as taken, 
    * instructions are overwritten mid-flight by this function.
*/
